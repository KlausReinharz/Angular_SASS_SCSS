...
export class AutocompleteComponent implements OnDestroy, ControlValueAccessor {
  ...
  ngOnInit(): void {
    ...
    this.formControl.valueChanges.pipe(takeUntil(this.destroy), distinctUntilChanged()).subscribe(items=>{
      const value = typeof items ==='object' ? items.value: null;
  ...
  writeValue(value: Value): void {
    const selectOption = this.items.find(items=>items.value===value);
    this.formControl.setValue(selectOption);
  }
